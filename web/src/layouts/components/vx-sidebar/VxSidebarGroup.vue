  ...
  <div
	...
	<div @click="clickGroup" class="group-header w-full">
		<span class="flex items-center w-full">
			<feather-icon :icon="group.icon || 'CircleIcon'"  v-if="group.icon || (this.groupIndex > Math.floor(this.groupIndex))" />
    ...
    <ul ref="items" :style="styleItems" class="vs-sidebar-group-items pl-5">
      <li v-for="(groupItem, index) in group.submenu" :key="index">
		...
		<vx-sidebar-item :index="groupIndex + '.' + index" :to="groupItem.url" svgClasses="w-3 h-3" :icon="groupItem.icon"  :target="groupItem.target" v-else>
