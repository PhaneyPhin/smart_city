...
export default class Auth extends Jwt {
    ...
    public Login = async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (username == undefined || password == undefined) {
            ...
            try {
                var users: any = await this.getOfDB(`select username,first_name,last_name,role_name from user_master u 
                                                            inner join role_master r  on u.role_id=r.role_id 
                                                     where username=$1 and password=SHA1($2) and flag='1'`, [username, password]);
                if (users.length > 0) {
                    var user = users[0];
